...
class SqliteTransactionRepository implements ITransactionRepository {
  ...
  Future<List<TransactionItem>> fetchTransactions({String? startDate, String? endDate, String? accountId}) async {
    ...
    return maps.map((m) => TransactionItem(
      ...
      toAccount: m['to_account_id'] != null
          ? AccountItem(
              id: m['to_account_id'],
              name: m['to_account_name'] ?? 'Account ${m['to_account_id']}',
              amount: Amount(0, 'EUR', false),
              flags: 0,
            )
          : null,
      category: m['category_id'] != null
          ? Category(
              id: m['category_id'],
      ...
      for (var tx in transactions) {
        await txn.rawInsert(
          '''INSERT OR REPLACE INTO transactions
             (server_id, amount_value, amount_currency, is_negative, type, description, date,
              account_id, account_name, category_id, category_name, subcategory_id,
              to_account_id, to_account_name, pending_sync)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
          [
            tx.id,
            tx.amount.value,
